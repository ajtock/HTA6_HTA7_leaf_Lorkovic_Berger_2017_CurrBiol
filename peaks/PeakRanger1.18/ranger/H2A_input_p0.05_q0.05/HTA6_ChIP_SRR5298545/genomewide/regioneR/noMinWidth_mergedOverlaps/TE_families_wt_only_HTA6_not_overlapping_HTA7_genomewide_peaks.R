...
# Usage:
# ./TE_families_wt_only_HTA6_not_overlapping_HTA7_genomewide_peaks.R "Genome-wide HTA6 peaks not overlapping HTA7 peaks" "HTA6_not_HTA7" HTA6_not_overlapping_HTA7 HTA6_not_overlapping_HTA7 genomewide 10000

library(ggplot2)
library(ggthemes)

#plotTitle <- "Genome-wide HTA6 peaks not overlapping HTA7 peaks"
#pt1Name <- "HTA6_not_HTA7"
#pt1LibName <- "HTA6_not_overlapping_HTA7" 
#ptOrderLibName <- "HTA6_not_overlapping_HTA7"
#region <- "genomewide" 
...
ptOrderDir <- paste0("/home/ajt200/analysis/HTA6_HTA7_leaf_Lorkovic_Berger_2017_CurrBiol/peaks/PeakRanger1.18/ranger/H2A_input_p0.05_q0.05/HTA6_ChIP_SRR5298545/",
                     "genomewide/regioneR/noMinWidth_mergedOverlaps/")
pt1Dir <- paste0("/home/ajt200/analysis/HTA6_HTA7_leaf_Lorkovic_Berger_2017_CurrBiol/peaks/PeakRanger1.18/ranger/H2A_input_p0.05_q0.05/HTA6_ChIP_SRR5298545/",
...
otherNames <-  c("SPO11GR",
                 "nucleRnucsGR")
otherNamesPlot <- c("SPO11-1 hotspots", "Nucleosomes")
...
load(paste0(ptOrderDir, "permTest_", perms, "perms_",
            ptOrderLibName, "_genomewide_peaks_vs_TEsDNA.RData"))
ptOrder_DNA <- ptPeaksTEsDNAPerChrom
ptPeaksTEsDNAPerChrom <- NULL
load(paste0(ptOrderDir, "permTest_", perms, "perms_",
            ptOrderLibName, "_genomewide_peaks_vs_TEsRNA.RData"))
ptOrder_RNA <- ptPeaksTEsRNAPerChrom
ptPeaksTEsRNAPerChrom <- NULL
load(paste0(ptOrderDir, "permTest_", perms, "perms_",
            ptOrderLibName, "_genomewide_peaks_vs_Other.RData"))
ptOrder_Other <- ptPeaksOtherPerChrom
...
# Other
load(paste0(ptOrderDir, "permTest_", perms, "perms_",
            ptOrderLibName, "_", region , "_peaks_vs_Other.RData"))
pt1_Other <- ptPeaksOtherPerChrom
ptPeaksOtherPerChrom <- NULL
...
# pt1_Other
pt1_Other_Pval <- lapply(seq_along(pt1_Other), function(x) {
  pt1_Other[[x]]$numOverlaps$pval
})
pt1_Other_Obs <- lapply(seq_along(pt1_Other), function(x) {
  pt1_Other[[x]]$numOverlaps$observed
})
pt1_Other_Perm <- lapply(seq_along(pt1_Other), function(x) {
  pt1_Other[[x]]$numOverlaps$permuted
})
pt1_Other_Exp <- lapply(seq_along(pt1_Other), function(x) {
  mean(pt1_Other[[x]]$numOverlaps$permuted)
})
pt1_Other_log2ObsExp <- lapply(seq_along(pt1_Other_Obs), function(x) {
  log2((pt1_Other_Obs[[x]]+1)/(pt1_Other_Exp[[x]]+1))
})
pt1_Other_Zscore <- lapply(seq_along(pt1_Other), function(x) {
  pt1_Other[[x]]$numOverlaps$zscore
})
pt1_Other_AltHyp <- lapply(seq_along(pt1_Other), function(x) {
  pt1_Other[[x]]$numOverlaps$alternative
})
pt1_Other_alpha0.05 <- lapply(seq_along(pt1_Other_Perm), function(x) {
  if(pt1_Other_AltHyp[[x]] == "greater") {
    quantile(pt1_Other_Perm[[x]], 0.95)[[1]]
  } else {
    quantile(pt1_Other_Perm[[x]], 0.05)[[1]]
  }
})
pt1_Other_log2alpha0.05 <- lapply(seq_along(pt1_Other_alpha0.05), function(x) {
  log2((pt1_Other_alpha0.05[[x]]+1)/(pt1_Other_Exp[[x]]+1))
})
...
pt1_log2ObsExp_sorted <- c(pt1_log2ObsExp_sorted,
                           unlist(pt1_Other_log2ObsExp))
pt1_log2alpha0.05_sorted <- c(pt1_log2alpha0.05_sorted,
                              unlist(pt1_Other_log2alpha0.05))
pt1_famNames_sorted <- c(pt1_famNames_sorted,
                         otherNames)
pt1_famNamesPlot_sorted <- c(pt1_famNamesPlot_sorted,
                             otherNamesPlot)

# Combine in data.frame
df <- data.frame(Sample = rep(c(pt1Name),
                              each = length(pt1_log2ObsExp_sorted)),
                 Feature_category = rep(pt1_famNamesPlot_sorted, 1),
                 log2ObsExp = c(pt1_log2ObsExp_sorted),
                 log2alpha0.05 = c(pt1_log2alpha0.05_sorted))

df$Feature_category <- factor(df$Feature_category,
                              levels = pt1_famNamesPlot_sorted)
df$Sample <- factor(df$Sample,
                    levels = c(pt1Name))

bp <- ggplot(data = df,
             mapping = aes(x = Feature_category,
                           y = log2ObsExp,
                           fill = Sample)) +
  ...
  scale_fill_manual(name = "",
                    ...
                    labels = c(pt1Name)) +
  geom_point(mapping = aes(Feature_category, log2alpha0.05),
