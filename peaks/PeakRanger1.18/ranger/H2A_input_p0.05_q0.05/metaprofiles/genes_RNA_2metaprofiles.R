...
# ./genes_RNA_2metaprofiles.R 'Chr1,Chr2,Chr3,Chr4,Chr5' both 2000 2000 '2kb' 10 10bp '0.02,0.96' 'wt_RNAseq_Rep1,hta7_RNAseq_Rep1' '20180928_Pallas_RNAseq_series1/fastq_pooled/snakemake_RNAseq_STAR,20180928_Pallas_RNAseq_series1/fastq_pooled/snakemake_RNAseq_STAR' 'WT Rep1,hta7 Rep1' 'navy,firebrick1' 'TAIR10_chr_all' 'genomewide'
# conda deactivate
...
#                                        split = ",")))
#ChIPNames <- unlist(strsplit("wt_RNAseq_Rep1,hta7_RNAseq_Rep1",
#                             split = ","))
#ChIPNamesDir <- unlist(strsplit("20180928_Pallas_RNAseq_series1/fastq_pooled/snakemake_RNAseq_STAR,20180928_Pallas_RNAseq_series1/fastq_pooled/snakemake_RNAseq_STAR",
#                                split = ","))
#ChIPNamesPlot <- unlist(strsplit("WT Rep1,hta7 Rep1",
#                                 split = ","))
#ChIPColours <- unlist(strsplit("navy,firebrick1",
#                               split = ","))
...
ChIPNamesDir <- unlist(strsplit(args[10],
                                split = ","))
ChIPNamesPlot <- unlist(strsplit(args[11],
                                 split = ","))
ChIPColours <- unlist(strsplit(args[12],
                               split = ","))
refbase <- args[13]
regionName <- args[14]

yLabPlot <- "RNA-seq (CPM)"
...
ChIP_HTA7_genesMats <- mclapply(seq_along(ChIPNames), function(x) {
    as.matrix(read.table(paste0(ChIPDirs[x], "geneProfiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_", refbase, "_", align, "_sort_norm_HTA7_genes_", regionName,
...
ChIP_hta7_upreg_genesMats <- mclapply(seq_along(ChIPNames), function(x) {
    as.matrix(read.table(paste0(ChIPDirs[x], "geneProfiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_", refbase, "_", align, "_sort_norm_hta7_upreg_genes_", regionName,
                                "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
}, mc.cores = length(ChIPNames))
...
ChIP_genesMats <- mclapply(seq_along(ChIPNames), function(x) {
    as.matrix(read.table(paste0(ChIPDirs[x], "geneProfiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_", refbase, "_", align, "_sort_norm_genes_", regionName,
                                "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
}, mc.cores = length(ChIPNames))
# genes_ranLoc
...
log2ChIP_HTA7_genes_ranLocMats <- mclapply(seq_along(ChIP_HTA7_genes_ranLocMats), function(x) {
  log2((ChIP_HTA7_genes_ranLocMats[[x]]+1))
...
log2ChIP_hta7_upreg_genesMats <- mclapply(seq_along(ChIP_hta7_upreg_genesMats), function(x) {
  log2((ChIP_hta7_upreg_genesMats[[x]]+1))
...
ggsave(paste0(plotDir,
              ...
              "_avgProfiles_around",
              "_HTA7_genes__hta7_upreg_genes__all_genes_", regionName, "_", refbase,
              "_", align, ".pdf"),
